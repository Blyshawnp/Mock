...
public partial class SupervisorTransferViewModel : ViewModelBase
{
    ...
    private void RefreshFromSession()
    {
        ...
        var transfer1Passed = (session.Transfers ?? []).FirstOrDefault(x => x.AttemptNumber == 1)?.Outcome == Core.Models.Enums.TransferOutcome.Pass;
        foreach (var transferVm in Transfers)
        {
            transferVm.IsVisible = transferVm.AttemptNumber != 2 || !transfer1Passed;
        }
