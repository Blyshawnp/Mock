...
public sealed class SessionStateService : ISessionStateService
{
    ...
    public void UpdateSessionInfo(string evaluatorName, string candidateName, bool finalAttempt)
    {
        MarkStateMutated();
        CurrentSession.EvaluatorName = evaluatorName;
        CurrentSession.CandidateName = candidateName;
        CurrentSession.FinalAttempt = finalAttempt;

        if (CurrentSession.Status == SessionStatus.Draft)
        {
            CurrentSession.Status = SessionStatus.InProgress;
            CurrentSession.CurrentScreenKey = "Calls";
        }
    ...
    public void UpdateCall(CallRecord updatedCall)
    {
        ...
        existing.DonorScenario = updatedCall.DonorScenario;
